...
const DraftList = ({ drafts, handleOnClick }) => {
  const draftList = drafts.map((draft) => {
    return (
      <div
        key={draft._id}
        className="container relative shadow-md shadow-slate-600 bg-gray-100 p-3 rounded-md mb-4 overflow-hidden max-h-24"
      >
        ...
        <button
          className=" bg-emerald-500 rounded-lg p-1 absolute bottom-1 right-1 shadow-slate-700 shadow-md"
